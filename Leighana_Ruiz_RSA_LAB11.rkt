...
;; e must be coprime with lcm(p - 1, q - 1). (NOTE: lcm - the least common multiple is a bit more efficient than
;; simply the product (p-1)*(q-1). But the latter works as well and could be used instead).
...
(define (generate p q e)
  (define (mod-exp b e m)
    (cond ((= e 0) 1)
          ((even? e) (remainder (expt (mod-exp b (/ e 2) m) 2) m))
          (else (remainder (* b (mod-exp b (- e 1) m)) m))))
...
(define (encrypt plaintext key)
  ...
  (let ((m (car key))
        (e (cadr key)))
    (map (lambda (char) (char-encrypt char key)) (string->list plaintext))))
...
(define (decrypt ciphertext key)
 (define (int-decrypt i key)
    (let* ((m (car key))
           (d (cadr key))
           (decrypted (mod-exp i d m)))
      (integer->char decrypted)))
  
  (let ((m (car key))
        (d (cadr key)))
    (list->string (map (lambda (int) (int-decrypt int key)) ciphertext))))
...
;; m = 145    e = 17          '(18 116 2 102 55 45 44 116 2 136 108 11 124 2 116 55 48 48 2 55 116 44 115 2 136 108 11 124)
