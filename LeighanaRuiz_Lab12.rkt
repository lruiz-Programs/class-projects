...
(define full5 '((1 1) (1 2) (1 3) (1 4) (1 5)
                      ...
                      (3 3) (3 4) (3 5)
                      (4 4) (4 5) (5 5)))
(define btree '((0 10) (0 11) (10 20) (10 21) (11 22) (11 23)
                        (20 30) (20 31) (21 32) (21 33)
                        (22 34) (22 35) (23 36) (23 37)))
(define triforce '((0 10) (0 11) (10 11) (10 20) (10 21)
...
;; Given a graph, a list "found" to serve as an auxiliary container for a found set of vertices,
;;   and a starting vertex v,
;;   dfs returns the list of all vertices which can be reached from v
...
(define (get-vertices G)
  (define (get-vertices-helper G)
    (if (null? G)
        '()
        (append (car G) (get-vertices-helper (cdr G)))))
...
(define (degree G v)
  (define (count-occurrences lst target)
    (if (null? lst)
        0
        (+ (if (equal? (car lst) target) 1 0)
           (count-occurrences (cdr lst) target))))

  (if (null? G)
      0
      (let ((edge (car G)))
        (if (equal? edge (list v v))
            (+ 2 (degree (cdr G) v))
            (+ (count-occurrences edge v)
               (degree (cdr G) v))))))
...
(define (all-even? G)
  ...
  (define (all-even-helper vertices)
    (if (null? vertices)
        #t
        (let ((v (car vertices)))
          (if (even? (degree G v))
              (all-even-helper (cdr vertices))
              #f))))

  (all-even-helper (usort (get-vertices G))))
...
(define (adjacent G v)
  (if (equal? G '())
      '()
      (if (equal? (car G) (list v v))
          (let ((result (adjacent (cdr G) v)))
            (append (list v) (usort result)))
          (if (equal? (car (car G)) v)
              (let ((result (adjacent (cdr G) v)))
                (append (list (car (cdr (car G)))) (usort result)))
              (if (equal? (car (cdr (car G))) v)
                  (let ((result (adjacent (cdr G) v)))
                    (append (list (car (car G))) (usort result)))
...
;;   from the input graph, and call "dfs"
...
(define (connected? G)
  (if (equal? G '())
      #t
      (if (equal? (usort (dfs G (list (car (car G))) (car (car G)))) (get-vertices G))
          #t
          #f)))
...
;;   (eulerian? empty) -> #t
;;
;; Type signature: (eulerian? graph) -> bool(define (eulerian? G)
(define (eulerian? G)
